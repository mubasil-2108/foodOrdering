...
const Search = ({ windowSize, searchFood, setSearch }) => {
    const isMobile = windowSize.width < 768;

    return (
        <StyledWrapper>
            <div className="searchBox" style={{ width: isMobile ? '100%' : '400px' }}>
                <input className="searchInput" onChange={(e) => setSearch(e.target.value)} type="text" name placeholder="Enter Dishes" />
                <button className="searchButton" onClick={searchFood}>
...
const StyledWrapper = styled.div`
  ...
  .searchButton {
    ...
    height: 50px;
    border-radius: 50%;
    background: var(--gradient-2, linear-gradient(90deg, #f9e0b0 0%, orange 100%));
    border: 0;
